package com.example.rootread.utils
...
object SecurityUtils {
    ...
    fun isAppTampered(context: Context): Boolean {
        Log.d("SOLUTION","isAppTampered")
        val expectedPackageName = "com.example.rootread"
...
//        if (!verifyAppSignature(activity) || isAppTampered(activity) || checkSuCommand() || isDebuggerAttached() || isBeingTraced() || isDeviceRooted()) {
...
//        }
        ...
        if (isAppTampered(activity)) {
            Log.d("SECURITY_CHECK", "isAppTampered(activity) ${isAppTampered(activity)}")
        }
        if (checkSuCommand()) {
